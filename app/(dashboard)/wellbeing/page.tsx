...
const SCORE_QUESTIONS = [
  { key: "energy",  icon: Zap,     color: "from-amber-500 to-orange-500" },
  { key: "sleep",   icon: Moon,    color: "from-indigo-500 to-violet-500" },
  { key: "focus",   icon: Target,  color: "from-cyan-500 to-blue-500" },
  { key: "stress",  icon: Brain,   color: "from-rose-500 to-pink-500" },
  { key: "purpose", icon: Sun,     color: "from-emerald-500 to-teal-500" },
  { key: "social",  icon: Users,   color: "from-fuchsia-500 to-purple-500" },
] as const;
...
export default function WellbeingPage() {
  ...
  const ratings = t.raw("ratings") as string[];
  ...
  const verdict =
    pct >= 75 ? { label: t("verdicts.thrive"),       color: "from-emerald-500 to-teal-500",  border: "border-emerald-500/30 bg-emerald-500/5" }
    : pct >= 50 ? { label: t("verdicts.stable"),     color: "from-amber-500 to-orange-500",  border: "border-amber-500/30 bg-amber-500/5" }
    : pct >= 30 ? { label: t("verdicts.stressed"),   color: "from-orange-500 to-red-500",    border: "border-orange-500/30 bg-orange-500/5" }
    : { label: t("verdicts.needs_support"),           color: "from-rose-500 to-pink-500",     border: "border-rose-500/30 bg-rose-500/5" };
      ...
      <div dir="rtl" className="max-w-3xl mx-auto space-y-6 pb-16">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-white/50 max-w-xl text-sm">{t("pageSubtitle")}</p>
          </div>
          <Link
            href="/dashboard"
          ...
          {mode === "home" && (
            <motion.div key="home" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  onClick={() => { setMode("assessment"); setStep(0); }}
                ...
                >
                  ...
                  <h3 className="font-bold text-white text-lg mb-1">{t("assessmentTitle")}</h3>
                  <p className="text-white/50 text-sm">{t("assessmentDesc")}</p>
                  <div className="mt-4 text-xs text-rose-400 font-medium">{t("assessmentDuration")} ←</div>
                </button>
                <button
                ...
                >
                  ...
                  <h3 className="font-bold text-white text-lg mb-1">{t("checkinTitle")}</h3>
                  <p className="text-white/50 text-sm">{t("checkinDesc")}</p>
                  <div className="mt-4 text-xs text-violet-400 font-medium">{t("checkinDuration")} ←</div>
                </button>
              </div>
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-white">{t("whyTitle")}</span>
                </div>
                <div className="grid grid-cols-3 gap-3 text-sm text-white/60">
                  {[
                    { n: t("stat1n"), text: t("stat1t") },
                    { n: t("stat2n"), text: t("stat2t") },
                    { n: t("stat3n"), text: t("stat3t") },
                  ].map(({ n, text }) => (
                ...
                {SCORE_QUESTIONS.map((_, i) => (
                  <div
                    ...
                    className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                      i < step ? "bg-rose-500" : i === step ? "bg-white/60" : "bg-white/10"
                    }`}
                  />
                ))}
              </div>
                ...
                return (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8 space-y-6">
                    ...
                    <div>
                      ...
                      <p className="text-white/60 text-sm">{t(`qSubs.${q.key}`)}</p>
                    </div>
                    <div className="grid grid-cols-5 gap-2">
                      {[1, 2, 3, 4, 5].map(v => (
                        <button
                          ...
                          className={`rounded-xl border-2 p-4 flex flex-col items-center gap-1.5 transition-all hover:scale-105 ${
                            scores[q.key] === v
                              ? "border-rose-500 bg-rose-500/20 text-white"
                              : "border-white/10 bg-white/[0.03] text-white/40 hover:border-rose-500/40 hover:text-white/80"
                          }`}
                        >
          ...
          {mode === "checkin" && (
            <motion.div key="checkin" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              <div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-6">
                <h2 className="font-bold text-white text-xl mb-2">{t("checkinPrompt")}</h2>
                <p className="text-white/50 text-sm mb-4">{t("checkinHint")}</p>
                <textarea
                  value={checkinMood}
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={sendCheckin}
                    disabled={!checkinMood.trim() || checkinLoading}
                  ...
                  >
                    ...
                    {checkinLoading ? t("analyzingBtn") : t("sendBtn")}
                  </button>
                  <button onClick={reset} className="text-sm text-white/30 hover:text-white/60 transition-colors">
                    {t("back")}
                  </button>
              ...
              {checkinResult && (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    <Heart className="w-4 h-4 text-rose-400" />
                    <span className="text-sm font-semibold text-white">{t("coachLabel")}</span>
                  </div>
            ...
            <motion.div key="results" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
              <div className={`rounded-2xl border p-6 ${verdict.border}`}>
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <div className="text-xs text-white/50 mb-1">{t("yourResult")}</div>
                    <div className={`text-3xl font-black bg-gradient-to-r ${verdict.color} bg-clip-text text-transparent`}>
                      {verdict.label}
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-5xl font-black text-white">{pct}٪</div>
                    <div className="text-xs text-white/40 mt-1">{t("wellbeingScore")}</div>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {SCORE_QUESTIONS.map(q => (
                    <div key={q.key} className="rounded-xl bg-white/[0.05] p-3 text-center">
                      ...
                      <div className="text-[10px] text-white/50 mt-0.5">{t(`qLabels.${q.key}`)}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                <label className="block text-sm text-white/60 mb-2">{t("contextLabel")}</label>
                <textarea
                  value={context}
                ...
                >
                  ...
                  {loading ? t("analyzeLoading") : t("analyzeBtn")}
                </button>
              </div>
              ...
              {aiResult && (
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm font-semibold text-white">{t("analysisLabel")}</span>
                  </div>
