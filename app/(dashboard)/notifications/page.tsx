      ...
      <div className="max-w-2xl mx-auto py-6 px-2 sm:px-0" dir="rtl">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            {unread > 0 && (
              <p className="text-sm text-neutral-400 mt-1">{unread} إشعار غير مقروء</p>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={load}
              className="p-2 rounded-xl text-neutral-400 hover:text-white hover:bg-white/[0.05] transition-all"
            >
            ...
            {unread > 0 && (
              <button
                onClick={markAll}
                className="flex items-center gap-1.5 text-xs font-semibold text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 px-3 py-1.5 rounded-xl transition-all"
              >
                    ...
                    {unreadItems.map((item) => {
                      ...
                      return (
                        <motion.div
                          ...
                          exit={{ opacity: 0, x: 20 }}
                          className={cn(
                            "rounded-2xl border p-4 flex gap-3 group transition-all hover:border-opacity-60",
                            cfg.bg, cfg.border
                          )}
                        >
                          ...
                          <button
                            onClick={() => markOne(item.id)}
                            className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-white/10 text-neutral-600 hover:text-emerald-400 transition-all shrink-0"
                            title="تحديد كمقروء"
                          >
                            <CheckCheck className="w-3.5 h-3.5" />
                          </button>
                ...
                <div className="space-y-2">
                  {readItems.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="glass-panel rounded-2xl p-4 flex gap-3 opacity-60 hover:opacity-80 transition-opacity"
                      >
