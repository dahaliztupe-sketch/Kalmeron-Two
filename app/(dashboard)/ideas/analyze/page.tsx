    ...
    <AppShell>
      <div className="max-w-4xl mx-auto space-y-8 p-4 text-white" dir={dir}>
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-4xl font-black tracking-tight mb-3 flex items-center gap-3 text-[rgb(var(--gold))]">
            <Lightbulb className="h-10 w-10 text-[rgb(var(--gold))]" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <CardContent className="space-y-6 flex flex-col p-8">
                ...
                <Textarea 
                    ...
                    className="min-h-[160px] resize-none text-lg p-5 rounded-2xl bg-neutral-900/50 text-white border-neutral-700 focus-visible:ring-[rgb(var(--gold))]"
                />
                </div>
                <Button 
                onClick={handleValidate} 
                disabled={!ideaDesc.trim() || isValidating}
                className="w-full h-14 text-lg rounded-xl bg-[rgb(var(--gold))] text-black hover:bg-[#d9a31a] font-bold shadow-lg transition-transform md:w-1/3 self-end"
                >
                {isValidating ? (
        ...
        {validationResult && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
            <Card className="glass border-[rgba(244,180,26,0.3)] shadow-[0_0_30px_rgba(244,180,26,0.1)] rounded-3xl overflow-hidden mt-8">
                <CardHeader className="bg-[rgb(var(--gold))]/10 border-b border-[rgba(244,180,26,0.2)] p-8">
                <CardTitle className="flex items-center gap-3 text-2xl text-[rgb(var(--gold))]">
                    <ChevronRight className="h-6 w-6 icon-flip" />
                    التحليل الاستراتيجي وخارطة الطريق
                </CardTitle>
                </CardHeader>
                <CardContent className="p-8">
                <div className="markdown-body text-lg leading-relaxed prose prose-invert prose-neutral max-w-none prose-headings:text-[rgb(var(--gold))] prose-a:text-blue-400" dir="auto">
