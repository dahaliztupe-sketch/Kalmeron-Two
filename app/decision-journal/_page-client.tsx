    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-auto" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="bg-[#0a0d14] border border-white/10 rounded-2xl p-6 max-w-2xl w-full my-auto" dir="rtl">
        ...
        <div className="space-y-3">
          <FormField label="عنوان مختصر للقرار *"><input aria-label="مثال: توظيف مطوّر full-time أم contractor" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="مثال: توظيف مطوّر full-time أم contractor" className="form-input" /></FormField>
          ...
          <FormField label="ما اخترت *"><input aria-label="حقل إدخال" value={chosen} onChange={(e) => setChosen(e.target.value)} className="form-input" /></FormField>
