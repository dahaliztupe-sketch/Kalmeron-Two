...
async function pingService(url: string): Promise<{ ok: boolean; [k: string]: unknown }> {
  // `url` is always one of PDF_WORKER_URL / EGYPT_CALC_URL / LLM_JUDGE_URL /
  // EMBEDDINGS_WORKER_URL — operator-controlled env vars (default localhost:PORT).
  // The path appended is the fixed literal "/health". No user input reaches here.
