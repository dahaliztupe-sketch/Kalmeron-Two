...
- Arabic: "أنا ذاهب للنوم. استخدم GNHF مع Claude لإكمال العمل على هذا الفرع وتوقف عندما تجتاز مجموعة الاختبارات."
...
- Arabic: "شغّل GNHF عدة جولات على تدفق onboarding هذا. افحص الـ diff بين الجولات وضيّق الـ prompt التالي إذا بدأ يتشتت."
...
## Kalmeron-Specific Launch Examples

### Fix TypeScript errors overnight:
```bash
gnhf \
  --agent claude \
  --max-iterations 15 \
  ...
  --prevent-sleep on \
  "Fix all TypeScript errors in this Kalmeron Next.js 16 project. Run npx tsc --noEmit after each fix. Do not change business logic. Stop when tsc reports zero errors."
```

### Improve Arabic AI agent responses:
```bash
gnhf \
  --agent claude \
  --max-iterations 20 \
  --stop-when "all agent prompts in lib/agents/ have Arabic system messages and pass lint" \
  --prevent-sleep on \
  "Improve the Arabic system prompts for all Kalmeron agents in lib/agents/. Ensure each agent has clear, professional Arabic instructions. Run npm run lint after changes. Stop when all agents have quality Arabic prompts and lint passes."
```
...
gnhf \
  --agent claude \
  --max-iterations 25 \
  --stop-when "npm run test exits with all tests passing and coverage above 70%" \
  --prevent-sleep on \
  "Add Vitest unit tests for the utility functions in lib/utils/ and lib/agents/ in this Kalmeron project. Run npm run test after each addition. Stop when test coverage reaches 70% and all tests pass."
```
...
Use when the user returns with "good morning", "كيف سارت الليلة؟", "how did last night's run go?", or similar after a GNHF run.
...
## Agent Selection for Kalmeron

- `claude`: reasoning-heavy implementation, Arabic text quality, prompt engineering, or prose-heavy planning.
...
**Default for Kalmeron**: use `claude` unless the user specifies otherwise.
...
- For Kalmeron: never modify Firestore security rules, billing logic, or authentication flows in a Hands-Off run without explicit authorization.
