...
10 أسئلة قوية بأسلوب Mom Test للسوق المصري تحديداً:
- 3 أسئلة عن الألم الحالي
- 3 أسئلة عن السلوك الفعلي
- 2 أسئلة عن المحاولات السابقة لحل المشكلة
- 2 أسئلة عن الاستعداد للدفع
...
${answersText ? `## 📝 تحليل مقابلاتك الحالية\nماذا تقول هذه الإجابات؟ ما الأنماط؟ ما الخطوة التالية؟` : '## 🎯 خطة الـ10 مقابلات الأولى\nكيف تجد ومن أين تبدأ المقابلات في مصر؟'}
  ...
  return instrumentAgent(
    ...
    async () => {
      ...
      const { text } = await generateText({
        model: MODELS.FLASH,
        system,
        prompt: `اكتب سكريبت مقابلة (15-20 دقيقة) لاكتشاف عملاء "${businessIdea}" من شريحة "${targetSegment}" في مصر. اجعله طبيعياً وودوداً.`,
      });
      return text;
    },
