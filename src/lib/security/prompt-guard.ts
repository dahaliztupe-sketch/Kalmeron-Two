...
export function sanitizeInput(input: string): string {
  ...
  // محاولات Markdown لإخفاء توجيهات داخل تعليقات
...
const INJECTION_PATTERNS: readonly InjectionPattern[] = [
  ...
  { name: 'ar_ignore',
    regex: /(?:تجاهل|تجاوز|تخطّ|تخط|انس(?:ى)?|الغ(?:ي|اء)?|أبطل|تخلّ\s+عن|اصرف\s+النظر\s+عن)[\s\u0600-\u06FFa-zA-Z]{0,40}?(?:كل\s+التعليمات|التعليمات|الأوامر|القواعد|التوجيهات|البرومبت|الإرشادات|التوجّهات|التوجهات|التعليمات\s+السابقة|التعليمات\s+الأولى|التعليمات\s+التي|قواعدك)/i,
    weight: 0.95 },
  { name: 'ar_new_role',
    regex: /(?:أنت\s+(?:الآن|من\s+الآن)|دورك\s+الجديد|تظاهر(?:\s+أنك)?|تخيل\s+أنك|العب\s+دور|تقمّص(?:\s+شخصية)?|كن\s+(?:الآن)?)/i,
    weight: 0.85 },
  ...
  { name: 'ar_reveal_hidden_rules',
    ...
    weight: 0.95 },
  { name: 'ar_no_filter',
    regex: /(?:بدون|دون|بلا|من\s+غير)[\s\u0600-\u06FFa-zA-Z]{0,15}?(?:قيود|رقابة|فلتر|حدود|قواعد|أخلاق|ضوابط)/i,
    weight: 0.9 },
  { name: 'ar_stop_being',
