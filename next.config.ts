...
const nextConfig: NextConfig = {
  ...
  // ── Output File Tracing (top-level — مطلوب في Next.js 15+) ───────────────
  // @vercel/nft يفحص الكود بعد البناء لتحديد الملفات اللازمة لكل serverless function.
  // بعض مجلدات المشروع (مثل .agents/skills/) تحتوي ملفات SKILL.md تُقرأ ديناميكياً
  // في runtime-loader.ts عبر path.join(SKILLS_ROOT, relPath) — وهذا يُربك NFT لأنه
  // يحاول تتبّع المسارات النسبية من جذر المشروع بدلاً من .agents/skills/ مما يُنتج
  // أخطاء ENOENT (engineering-team/adversarial-reviewer/SKILL.md، إلخ).
  //
  // الحل الجذري: استبعاد المجلدات الداخلية من التتبع. runtime-loader.ts يتعامل
  // بأمان مع الملفات الغائبة عبر existsSync (يُرجع null بدلاً من الانهيار).
